...
class Picture(Matrix):
    ...
    def run(self):
        ...
        if lowercase_name.startswith('affirmations'):
            ...
            self.thread = StoppableThread(target=self.display_affirmations, args=(name,))
            ...
            print(f'Cycling through everything from {lowercase_name}! Only cycling through images for now')
            gif = False
            for f in os.listdir(f'{DIR}/images/{lowercase_name}'):
                if f.split('.')[-1].lower() == 'gif':
                  ...
                  break
            if gif:
                self.thread = StoppableThread(target=self.cycle_dir_gifs, args=(lowercase_name,))
                self.thread.start()
            else:
                self.thread = StoppableThread(target=self.cycle_dir_imgs, args=(lowercase_name,))
                self.thread.start()
            return
        ...
        if file is None:
            ...
            self.thread = StoppableThread(target=self.scroll_text, args=(text_to_display, 0.04))
    ...
    def gif_n(self, frames, num_frames, iters, framerate):
        cur_frame = 1
        i = 0
        while i < iters:
    ...
    def display_affirmations(self, name='You'):
        ...
        while True:
            ...
            length2 = graphics.DrawText(self.canvas, font, x_pos, 44+64, color, aff_word)

            # Pause at the halfway point
            if x_pos + length1//2 > self.canvas.width//2 and wait_counter < WAIT_TIME:
                wait_counter += 1
    ...
    def display_help(self):
        ...
        special_commands_str = ' | '.join(["special commands: affirmations | help | howto | random"] + self.all_subdirs)
        print(special_commands_str)
        all_full_paths = list(self.paths.values())
        pngs_available = [(x.split('/')[-1])[:-(len('.png'))] for x in all_full_paths if x.endswith('.png')]
        gifs_available = [(x.split('/')[-1])[:-(len('.gif'))] for x in all_full_paths if x.endswith('.gif')]
        ...
        while True:
            ...
            self.canvas.Clear()
            lengths = [
                graphics.DrawText(self.canvas, font, x_positions[0], 14, colors[0], special_commands_str),
                graphics.DrawText(self.canvas, font, x_positions[1], 32, colors[1], pngs_str),
                graphics.DrawText(self.canvas, font, x_positions[2], 50, colors[2], gifs_str),
            ]
            # Backside
            graphics.DrawText(self.canvas, font, x_positions[0], 14+64, colors[0], special_commands_str),
            graphics.DrawText(self.canvas, font, x_positions[1], 32+64, colors[1], pngs_str),
            graphics.DrawText(self.canvas, font, x_positions[2], 50+64, colors[2], gifs_str),
